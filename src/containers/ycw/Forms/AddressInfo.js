...
const AddressInfo = () => {
  ...
  const { register, handleSubmit, reset, watch, setValue, getValues, trigger } =
    useForm({
      defaultValues: {
        ...
        permanentAddress: { ...ADDRESS_INFO_FORM_FIELDS },
      },
    ...
    axios
      .all(
        [
          ...
          COUNTRY,
          `${STATE_BY_ID}${
            updatedFormFields?.currentAddress?.countryUuid ?? '1'
          }`,
        ].map((url) => Axios.get(url))
      )
      .then(
        axios.spread(
          ({ data: addressProof }, { data: country }, { data: state }) =>
            setDropDownList((prevState) => ({
              ...prevState,
  ...
  const handleIsPermanent = (check) => {
    ...
    const currentAddress = {
      ...updatedFormFields.currentAddress,
      permanent: true,
    };
    const permanentAddress = check
      ? { ...updatedFormFields.currentAddress, permanent: true }
      : {};
    // update form
    reset({
      currentAddress,
    ...
    <form onSubmit={handleSubmit(() => handleSave(true))}>
      ...
      <CurrentAdd
        ...
        handleAddress={({ target: { name, value } }) => {
          ...
          handleDropDownAPIs('permanentAddress', {
            [name.split('.')[1]]: value,
          });
          setValue(name, value);
        }}
